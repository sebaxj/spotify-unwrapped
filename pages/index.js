...
export default function Home() {
  ...
  const getLocations = async () => {
    ...
    const filteredList = list.filter(
      (item) => !item.artists[0].hometown && !item.artists[0].area
    );

    const cachedList = list.filter(
      (item) => item.artists[0].hometown || item.artists[0].area
    );
    setMarkers(
      cachedList.map((item) => {
    ...
    filteredList.forEach(async (item, index) => {
      ...
      const res = await fetch(
        `https://musicbrainz.org/ws/2/artist?fmt=json&query=${item.artists[0].name}&limit=1`
      );
      ...
      const hometownName = hello.artists[0]["begin-area"]
        ? hello.artists[0]["begin-area"].name
        : null;
      const areaName = hello.artists[0].area
        ? hello.artists[0].area.name
        : null;

      if (hometownName) {
        const mapres = await fetch(
          `https://api.mapbox.com/geocoding/v5/mapbox.places/${hometownName}.json?types=${getMapBoxTypesFromMusicBrainz(
            hello.artists[0]["begin-area"]
          )}&access_token=${process.env.NEXT_PUBLIC_MAPBOX_ACCESS_TOKEN}`
        );
        const mapdata = await mapres.json();
        if (mapdata.features[0]) {
          hometown = {
            ...mapdata.features[0].geometry,
            name: mapdata.features[0].place_name,
          };
        }
      }
      ...
      if (areaName) {
        const mapres = await fetch(
          `https://api.mapbox.com/geocoding/v5/mapbox.places/${areaName}.json?types=${getMapBoxTypesFromMusicBrainz(
            hello.artists[0].area
          )}&access_token=${process.env.NEXT_PUBLIC_MAPBOX_ACCESS_TOKEN}`
        );
        const mapdata = await mapres.json();
        if (mapdata.features[0]) {
    ...
    <>
      ...
      <div style={{ width: "100vw", height: "100vh" }}>
        <Map
          ...
          showGlobalPlaylists={showGlobalPlaylists}
        />
      </div>
      ...
      <SearchBar
        ...
        showGlobalPlaylists={showGlobalPlaylists}
        setShowGlobalPlaylists={setShowGlobalPlaylists}
      />
      <Sidebar
        loading={loading}
        list={list}
