  ...
  switch (method) {
    case "GET":
      try {
        ...
        switch (playlistId) {
          case "recents":
            response = await spotifyApi.getMyRecentlyPlayedTracks({
              limit: 50,
            });
        ...
        const artists = await Artist.find({
          id: { $in: items.map((item) => item.artists[0].id) },
        });

        items = items.map((item) => {
          item.artists[0].hometown = artists.find(
            (artist) => artist.id === item.artists[0].id
          )?.hometown;
          item.artists[0].area = artists.find(
            (artist) => artist.id === item.artists[0].id
          )?.area;
          item.artists[0].image = artists.find(
            (artist) => artist.id === item.artists[0].id
          )?.images[0]?.url;
          return item;
        });
